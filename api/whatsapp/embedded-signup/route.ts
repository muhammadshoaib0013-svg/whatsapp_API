...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const redirectUri = process.env.META_REDIRECT_URI || `${process.env.NEXT_PUBLIC_APP_URL}/api/whatsapp/embedded-signup`;
    ...
    if (!tokenResponse.success) {
      return NextResponse.json(
        { error: 'Failed to exchange code for token', details: tokenResponse.error },
        { status: 500 }
      );
    }
    ...
    console.error('[EMBEDDED_SIGNUP] Error processing callback', { 
...
async function exchangeCodeForToken(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('[EMBEDDED_SIGNUP] Token exchange API error', {
    ...
    if (data.error) {
      console.error('[EMBEDDED_SIGNUP] Token exchange error in response', data.error);
      return {
        success: false,
    ...
    if (!wabaResponse.ok) {
      console.error('[EMBEDDED_SIGNUP] Failed to fetch WABA details');
      return {
        success: false,
    ...
    console.error('[EMBEDDED_SIGNUP] Exception during token exchange', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    };
